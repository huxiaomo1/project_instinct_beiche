...
from instinctlab.assets.unitree_g1 import (
    ...
    G1_29DOF_PELVISBASE_POPSICLE_CFG_URDF,
)
...
from instinctlab.tasks.parkour.config.parkour_env_cfg import ROUGH_TERRAINS_CFG, ROUGH_TERRAINS_CFG_stairs, ParkourEnvCfg

__file_dir__ = os.path.dirname(os.path.realpath(__file__))
# G1_CFG = copy.deepcopy(G1_29DOF_TORSOBASE_POPSICLE_CFG)
G1_CFG = copy.deepcopy(G1_29DOF_PELVISBASE_POPSICLE_CFG_URDF)
G1_CFG.spawn.merge_fixed_joints = True
G1_CFG.init_state.pos = (0.0, 20.0, 0.9)
...
class G1ParkourRoughEnvCfg_PLAY(G1ParkourRoughEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
        self.scene.terrain.terrain_generator = ROUGH_TERRAINS_CFG_stairs
        ...
        # Set fixed camera angle for play mode (in degrees)
        # Modify these values to test different camera angles
        camera_roll_deg = 0.87
        camera_pitch_deg = 48.0
        camera_yaw_deg = 0.0
        roll = torch.tensor(math.radians(camera_roll_deg))
        pitch = torch.tensor(math.radians(camera_pitch_deg))
        yaw = torch.tensor(math.radians(camera_yaw_deg))
        quat = math_utils.quat_from_euler_xyz(roll, pitch, yaw)
        self.scene.camera.offset.rot = tuple(quat.tolist())
